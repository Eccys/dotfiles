...
DEFAULT_DIR="$(xdg-user-dir)/Pictures/sss/$(date "+%Y-%m")"
...
shotocr() {
    ...
    if [[ -s "$tmpfile" ]]; then
        mv "$tmpfile" "$SAVE_DIR/$file"
        TEXT=$(tesseract "$SAVE_DIR/$file" - -l eng)
        echo "$TEXT" | wl-copy
        ${notify_cmd_shot} "Text copied to clipboard."
    else
...
if [[ "$1" == "--now" ]]; then
    ...
    echo -e "Available Options : --now --in5 --in10 --win --area --active --swappy --ocr"
fi
