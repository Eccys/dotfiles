...
end, { desc = "[P]ITALICIZE toggle italic markers" })
...
end, { desc = "[P]STRIKETHROUGH toggle strikethrough markers" })
...
vim.keymap.set("v", "<leader>me", function()
	...
	local selected_text = table.concat(lines, "\n"):sub(start_col, #lines == 1 and end_col or -1)
	if selected_text:match("^%~%~.~%~%~$") then
		vim.notify("Text already coded", vim.log.levels.INFO)
	else
		vim.cmd("normal 2gsa`")
	end
...
vim.keymap.set("n", "<leader>me", function()
	...
	-- Check if the cursor is on an asterisk
	if line:sub(col + 1, col + 1):match("%`") then
		vim.notify("Cursor is on an backtick, run inside the coded text", vim.log.levels.WARN)
		return
	end
	-- Search for '**' to the left of the cursor position
	local left_text = line:sub(1, col)
	...
	-- vim.notify("Unbolded text", vim.log.levels.INFO)
		...
		vim.notify("Coded current word", vim.log.levels.INFO)
	end
end, { desc = "[P]CODE toggle code markers" })
...
-- Bind Ctrl-Shift-K to insert a markdown link with clipboard content in normal mode
vim.keymap.set("n", "<C-S-k>", function()
    ...
    -- If there's a word under the cursor, format it as a markdown link
    if word_under_cursor ~= "" then
        -- Replace the word with markdown link format, keeping the word inside "[]"
        vim.cmd("normal! ciw[" .. word_under_cursor .. "](" .. clipboard_content .. ")")
    else
        -- Otherwise, insert the empty markdown link with clipboard content
        vim.api.nvim_put({ "[](" .. clipboard_content .. ")" }, "c", true, true)
    end
end, { desc = "Convert word to markdown link with clipboard content" })

-- Bind Ctrl-K to insert a markdown link without using the clipboard in normal mode
vim.keymap.set("n", "<C-k>", function()
    ...
    -- If there's a word under the cursor, format it as a markdown link
    if word_under_cursor ~= "" then
        -- Replace the word with empty markdown link format, keeping the word inside "[]"
        vim.cmd("normal! ciw[" .. word_under_cursor .. "]()")
    else
        -- Otherwise, insert the empty markdown link
        vim.api.nvim_put({ "[]()" }, "c", true, true)
    end
end, { desc = "Convert word to empty markdown link" })
